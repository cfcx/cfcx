...
layui.config({
	...
	var parseLayer = function (msg) {
		return layer.open({
			title: '晨曦云 (更新于：<b style="color:red">2021-07-13</b>)',
			type: 1,
			closeBtn: false,
			shadeClose: true,
			content: $('#lanzou'),
			...
			success: function (elem, index) {
				...
				$('input[name="id"]', elem).on('input propertychange', function () {
					var val = $(this).val();
					if (/[^a-z0-9\-]/.test(val)) $(this).val(val.replace(/[^a-z0-9\-]/g, ''));
					$('.layui-layer-btn0', elem).text(!/^b/i.test($(this).val()) ? window.isAdmin ? '设置' : '确定' : '解析');
				});

				$('input[name="pwd"]', elem).on('input propertychange', function () {
					var val = $(this).val();
					if (/^\s+|\s+$/.test(val)) $(this).val(val.replace(/^\s+|\s+$/g, ''));
				});

				$('input[name="id"]', elem).trigger('input');
			},
			yes: function (index, elem) {
				...
				if (!/^b/i.test(field.id) && window.isAdmin) {
